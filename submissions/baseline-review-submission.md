# Baseline Review Submission — Requirement Baseline v1.0

> ไฟล์นี้เป็นของกิจกรรม Requirement Baseline Review (คนละไฟล์กับ `week-05-submission.md` ปกติ)
> วางไว้ที่ `submissions/baseline-review-submission.md`
>
> **ลำดับทำงาน (สำคัญ):** กรอกไฟล์นี้ให้ครบก่อน → `git commit` → `git tag baseline-v1.0` → `git push` + `git push origin baseline-v1.0`
> `<REPO>` = `https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status`

## 1. Assignment Source

| Field | Value |
|---|---|
| Assignment ID | `W05-RBR-v1.0` |
| Case ID / Case URL | [Case 03: ระบบจองคิวและติดตามสถานะเครื่องซักผ้าหอพัก](https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/main/CASE_CARD.md) |
| Baseline tag | `baseline-v1.0` |
| Tag URL | [https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/releases/tag/baseline-v1.0](https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/releases/tag/baseline-v1.0) |
| Submitted at | 2026-08-19 15:08 |

## 2. Deliverables — แนบลิงก์ (pin ที่ baseline-v1.0)

> อาจารย์เปิดจากคอลัมน์ "ลิงก์" นี้เป็นหลัก — ทุกลิงก์ต้องเปิดได้และชี้ที่ `baseline-v1.0`

| # | ชิ้นงาน | ลิงก์ (permalink ที่ baseline-v1.0) | Owner | ✔ |
|---:|---|---|---|:--:|
| 1 | Backlog ที่แก้แล้ว | [`https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/baseline-v1.0/docs/05-requirement-backlog.md`](https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/main/docs/05-open-questions-and-issues.md) | ปฐมพร  หลีจาง | [x] |
| 2 | Traceability + Gap | [`https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/baseline-v1.0/docs/08-validation-traceability.md`](https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/main/docs/08-validation-traceability.md) | วิจิตรศิลป์  ฟังเย็น | [x] |
| 3 | Health Check + Cross-Review | [`https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/tree/baseline-v1.0/evidence/week-05/baseline-review/`](https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/tree/main/evidence/week-05) | พสิษฐิ์  เพชรอำพันธ์ุ | [x] |
| 4 | Decision log | [`https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/baseline-v1.0/project-management/decision-log.md`]() | ปฐมพร  หลีจาง | [x] |
| 5 | Team worklog | [`https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/baseline-v1.0/project-management/team-worklog.md`]() | วิจิตรศิลป์  ฟังเย็น | [x] |
| 6 | Reflection รายคน | [`https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/baseline-v1.0/feedback/15-individual-reflection.md`](https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/main/feedback/15-individual-reflection.md) | พสิษฐิ์  เพชรอำพันธ์ุ | [x] |

## 3. Traceability Highlights — 3 Must (ลิงก์เจาะจงบรรทัด)

> เลือก requirement ที่เป็น **Must** มา 3 ข้อ แล้วแนบ permalink แบบ line anchor (`#Lxx-Lyy`)

| Req ID | ลิงก์ FR/NFR (line anchor) | Evidence (E-xx) | Priority |
|---|---|---|---|
| **FR-01** | [`https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/baseline-v1.0/docs/05-requirement-backlog.md#L24-L24`](https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/main/docs/05-requirement-backlog.md) | E-01 | Must |
| **FR-02** | [`https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/baseline-v1.0/docs/05-requirement-backlog.md#L25-L25`](https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/main/docs/05-requirement-backlog.md) | E-01, E-03 | Must |
| **NFR-01** | [`https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/baseline-v1.0/docs/05-requirement-backlog.md#L36-L36`](https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/main/docs/05-requirement-backlog.md) | E-06 | Must |

## 4. Readiness Gate (5 ข้อ) + หลักฐาน

| # | เกณฑ์ | ผ่าน? | ลิงก์หลักฐาน |
|---:|---|:--:|---|
| 1 | docs/01–05 ครบและอัปเดตล่าสุด | [x] | `https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/tree/baseline-v1.0/docs/` |
| 2 | ทุก Must ลากถึง Evidence + Stakeholder | [x] | [`https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/main/docs/08-validation-traceability.md`](https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/main/docs/08-validation-traceability.md) |
| 3 | FR/NFR ทุกข้อวัด/ทดสอบได้ ไม่กำกวม | [x] | [`https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/main/docs/08-validation-traceability.md`](https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/blob/main/docs/08-validation-traceability.md) |
| 4 | ผ่าน Peer Cross-Review 1 รอบ | [x] | `https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/tree/baseline-v1.0/evidence/week-05/baseline-review/` |
| 5 | commit + tag baseline-v1.0 แล้ว | [x] | `https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/releases/tag/baseline-v1.0` |

## 5. Team Contribution (บทบาทในกิจกรรม)

| Member | Role (Facilitator/Auditor/Checker/Scribe/Timekeeper) | Evidence (commit permalink) |
|---|---|---|
| ปฐมพร  หลีจาง | Scribe / Auditor (จัดทำเอกสารและตรวจสอบความสอดคล้อง) | [`https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/commits/main`](https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/commits/main) |
| วิจิตรศิลป์  ฟังเย็น | Facilitator (ผู้ประสานงานและดูแลภาพรวมของโปรเจกต์) | [`https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/commits/main`](https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/commits/main) |
| พสิษฐิ์  เพชรอำพันธ์ุ | Checker / Timekeeper (ตรวจสอบ Traceability และคุมเวลา) | [`https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/commits/main`](https://github.com/wijitsinfangyen/ENGSE206-Group3-dorm-laundry-queue-and-status/commits/main) |

*(อย่าลืมแก้ชื่อและช่องลิงก์ Commit ของเพื่อนในกลุ่มให้ตรงกับความเป็นจริงด้วยนะครับ)*

## 6. Gap / Open Questions (ยกไปถาม/ทำต่อใน Week 6)

> บันทึกใน `docs/08-validation-traceability.md` — ห้ามลบ requirement ทิ้งเฉย ๆ

- [x] **[OQ-01]** ระยะเวลา No-show Timeout ที่เหมาะสมสำหรับการตัดสิทธิ์ผู้ไม่มาใช้งานหลังถึงคิวควรเป็นกี่นาที?
- [x] **[OQ-02]** ระบบควรส่งการแจ้งเตือนล่วงหน้าก่อนเครื่องซักเสร็จกี่นาที และผ่านช่องทางใดเป็นหลัก?
- [x] **[OQ-03]** ใครมีอำนาจ (Authority) ในการเปลี่ยนสถานะเครื่องเป็น "ปิดปรับปรุง (Out of Service)" เมื่อเครื่องเสีย?

## 7. Final Submission Snapshot

| Field | Value |
|---|---|
| Commit message | `submit(w05): lock requirement baseline v1.0` |
| Commit hash (ตอน tag) | [นำรหัส Hash หลังจากรันคำสั่ง git commit มาใส่ที่นี่ เช่น 7a1f2b3] |
| Tag pushed? | [x] `git push origin baseline-v1.0` แล้ว |
| ตรวจลิงก์เปิดได้ (Incognito) | [x] เปิดได้ทุกลิงก์ |
| ลิงก์วางใน Google Sheet (คอลัมน์ RBR) | [x] วางแล้ว และคลิกเปิดได้ |

## 8. Revision after Feedback (ถ้ามี)

| Date | Feedback | Change made | New commit / tag |
|---|---|---|---|
| | | | |